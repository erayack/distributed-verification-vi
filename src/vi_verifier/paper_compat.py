...
PAPER_COMPAT_ROWS: tuple[PaperCompatRow, ...] = (
    PaperCompatRow(
...
PAPER_COMPAT_BY_PREDICATE: dict[str, PaperCompatRow] = {
    row.predicate: row for row in PAPER_COMPAT_ROWS
}


def get_paper_compat_rows() -> list[PaperCompatRow]:
    return list(PAPER_COMPAT_ROWS)


def get_paper_compat_row(predicate: str) -> PaperCompatRow:
    try:
        return PAPER_COMPAT_BY_PREDICATE[predicate]
    except KeyError as exc:
        raise KeyError(f"unknown predicate in paper compatibility registry: {predicate}") from exc
...
def get_implemented_rows(include_approximated: bool = True) -> list[PaperCompatRow]:
    ...
    return [row for row in PAPER_COMPAT_ROWS if row.status in allowed]
